...
def explore(map:list[list[str]], starting_point:tuple[tuple[int,int], int], end_location:tuple[int,int]) -> PartialPath|None:
    neibs = [          (-1, 0), 
             ( 0, -1),          ( 0, 1), 
                       ( 1, 0)]
    ...
    while len(visiting_list) > 0:
        ...
        # clear()
        # copy_map = [list(''.join(line)) for line in map]
        # print_map(path.add_path(copy_map))
        # print(current_location, direction)
        # sleep(0.1)
...
def main():
    ...
    for i in range(len(map)):
        for j in range(len(map[0])):
            ...
            if map[i][j] == END:
                end_location = (i,j)
    res = explore(map, (start_location, EAST), end_location)
    if res:
        print(res.cost())
